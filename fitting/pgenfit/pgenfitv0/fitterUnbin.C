...
{
    Int_t nbins=20000/rebin;
    p_deadtime=deadtimeinput;
    ...
    RooRealVar ineueff("ineueff","ineueff",neueffinput,0.,1.) ;
    ineueff.setConstant(kTRUE);

    //fitF totdecaymodel("totdecaymodel","totdecaymodel",x,y,ineueff,"oldcompare-unbin/In134short.txt");
    fitF totdecaymodel("totdecaymodel","totdecaymodel",x,y,ineueff,parmsfile);
...
//    TH1F* hdecay2n=(TH1F*) gDirectory->Get(tempchar1);
    ...
    RooRealVar nsig("nsig","nsig",nnsig,nnsig/8,nnsig*8) ;
    nbkg.setConstant();    
    ...
    ofstream str("fitresultsunbinned.txt",ios::app);
    str<<outfilename<<"\t"<<TMath::Log(2)/totdecaymodel.getDecayParms(0)->getVal()*1000<<"\t"<<log(2)/totdecaymodel.getDecayParms(0)->getVal(0)/totdecaymodel.getDecayParms(0)->getVal(0)*totdecaymodel.getDecayParms(0)->getError()*1000<<fitres->status()<<endl;
