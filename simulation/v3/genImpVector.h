...
// Mon Jul  8 16:13:39 2019 by ROOT version 6.08/00
// from TTree treeIn134/treeIn134
// found on file: /home/phong/briken17/makerootfiles/rootfiles/decaynew_isomer_wab_neu/decay_brips3004.root
//////////////////////////////////////////////////////////

#ifndef genImpVector_h
#define genImpVector_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Int_t           gc_hit;
   Double_t        gc_E[11];   //[gc_hit]
   Double_t        gc_T[11];   //[gc_hit]
   Int_t           gc_ch[11];   //[gc_hit]
   Int_t           gc1_hit;
   Double_t        gc1_E[6];   //[gc1_hit]
   Double_t        gc1_T[6];   //[gc1_hit]
   Int_t           gc1_ch[6];   //[gc1_hit]
   Int_t           gc2_hit;
   Double_t        gc2_E[7];   //[gc2_hit]
   Double_t        gc2_T[7];   //[gc2_hit]
   Int_t           gc2_ch[7];   //[gc2_hit]
   Int_t           ab1_hit;
   ...
   Int_t           neu_hit;
   Double_t        neu_E[16];   //[neu_hit]
   Double_t        neu_T[16];   //[neu_hit]
   Int_t           neu_ch[16];   //[neu_hit]
   ...
   genImpVector(char* infile, char* riname, TTree *tree=0);
   virtual ~genImpVector();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop(Int_t layer, char* outfile);
...
#ifdef genImpVector_cxx
genImpVector::genImpVector(char* infile, char* riname, TTree *tree) : fChain(0)
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      ...
      f->GetObject((char*) Form("tree%s",riname),tree);

   }
   Init(tree);
}

genImpVector::~genImpVector()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t genImpVector::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t genImpVector::LoadTree(Long64_t entry)
{
...
void genImpVector::Init(TTree *tree)
{
...
void genImpVector::Show(Long64_t entry)
{
...
Int_t genImpVector::Cut(Long64_t entry)
{
