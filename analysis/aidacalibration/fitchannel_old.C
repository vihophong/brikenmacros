...
void fitchannel::Loop(Int_t dssd,Double_t ecut,Int_t npoint,char* outname)
{
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   //for (Int_t ii=0;ii<16384;ii++){
   for (Int_t ii=3842;ii<3852;ii++){
   //for (Int_t ii=3842;ii<3845;ii++){
       //! reject outer channels
       Int_t x_c=ii/128;
       Int_t y_c=ii%128;
       bool foolflag=false;
       ...
       if (!foolflag) continue;
       np++;
       std::map<double , double> emap;
       std::map<double , double>::iterator emap_it;
       emap.clear();
       //! fill in emap
       ...
       for (Long64_t jentry=0; jentry<nentries;jentry++) {
          ...
          Int_t index=x*128+y;
          if (z==dssd&&ex>ecut&&ey>ecut&&index==ii)
            emap.insert(std::make_pair(adcx,adcy));
       }
       if (emap.size()<npoint){//less data point
           ...
           //if (((Double_t)f->GetChisquare()/(Double_t)f->GetNDF())<100){
