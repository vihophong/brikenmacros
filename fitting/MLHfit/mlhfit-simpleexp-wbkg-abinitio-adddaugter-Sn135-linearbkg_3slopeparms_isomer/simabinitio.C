...
*/
  ...
  TFile *fintdiff = new TFile("tdiffdataSn135.root");
  TTree* treetdiff=0;
  fintdiff->GetObject("tree",treetdiff);
  Double_t        tdiff;
  treetdiff->SetBranchAddress("tdiff",&tdiff);
  Long64_t nentriestdiff = 20000;//treetdiff->GetEntries();
  Long64_t nbytes = 0;
...
//   for (Long64_t i=0; i<nentriestdiff;i++) {
//      nbytes += treetdiff->GetEntry(i);

//   }

  //while (tsimp<BeamTime+tsoffset){
   for (Long64_t i=110000; i<nentriestdiff+110000;i++) {
      nbytes += treetdiff->GetEntry(i);
      //! beam outside AIDA
