...
void simulatedecay(char* decaychainparmsfile, char* simparmsfile)
{
    ...
    while (!ifscond.eof()){
        ...
        if (line_head=="ishistgausbkg") isGaussianBkgfromhist=(Int_t)line_val;
        ...
        if (line_head=="neubkgrate") extbneukgrate=line_val;
    ...
    if (isSpatialDistFromHist!=0){
        cout<<"reading histograms hy.root, hx.root, himpx.root and himpy.root for beta background distribution"<<endl;
    ...
    std::ifstream ifs(decaychainparmsfile);
    Int_t rino;
    Double_t temp;
    Int_t knri=9;
  ...
  while (tsimp<BeamTime+tsoffset){
      ...
      if (ximp<xmax&&yimp<ymax&&ximp>=xmin&&yimp>=ymin){
          ...
          Double_t dtp1neui=r->Exp(beamneutronmodtime/log(2));
