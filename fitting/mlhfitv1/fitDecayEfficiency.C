...
void fitDecay(char* infile,char* histname,char* parmsfile,Int_t nimplants,Int_t rebin=0){
    ...
    TH1F* hdecay=(TH1F*) f->Get(histname);
   ...
   cout<<"\n\n******************************\n"<<endl;
   cout<<"Initial activity = "<<fB->GetParameter(19)<<endl;
   cout<<"Bin Width = "<<hB->GetBinWidth(1)<<endl;
   cout<<"Lambda = "<<fB->GetParameter(0)<<endl;
   cout<<"No. of Implant = "<<nimplants<<endl;

   cout<<"Efficiency = "<<fB->GetParameter(19)/hB->GetBinWidth(1)/fB->GetParameter(0)/nimplants*100<<" %\n\n"<<endl;

   TH1F * hbkg = (TH1F*) hdecay->Clone();
   hbkg->Reset();
   for (Int_t i=0;i<hbkg->GetNbinsX();i++){
       hbkg->SetBinContent(i+1,round(fB->GetParameter(20)));
   }
   hbkg->SetLineColor(2);
   hbkg->Draw("same");
   cout<<"All = "<<hB->Integral()<<endl;
   cout<<"Background = "<<hbkg->Integral()<<endl;
   hdecay->Add(hbkg,-1);
   cout<<"Integrated decay = "<<hdecay->Integral()<<endl;

   cout<<"Signal-to-background = "<<fB->GetParameter(19)/fB->GetParameter(20)<<endl;

}
void fitDecayEfficiency()
{
    //!  Syntax: fitDecay("input root file","histogram name","parameter file", number_of_implant)
    fitDecay("cd130hist.root","h1","parmsCd130.txt",5631);
}
