...
// Fri Aug 17 19:01:02 2018 by ROOT version 6.08/00
// from TTree treeIn133/treeIn133
// found on file: decay/decay_brips3107.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Int_t           neu_hit;
   Double_t        neu_E[6];   //[neu_hit]
   Double_t        neu_T[6];   //[neu_hit]
   Int_t           neu_ch[6];   //[neu_hit]
   Double_t        neu_x[6];   //[neu_hit]
   Double_t        neu_y[6];   //[neu_hit]
   Int_t           neub_hit;
   Double_t        neub_E[8];   //[neub_hit]
   Double_t        neub_T[8];   //[neub_hit]
   Int_t           neub_ch[8];   //[neub_hit]
   Double_t        neub_x[8];   //[neub_hit]
   Double_t        neub_y[8];   //[neub_hit]
...
#ifdef treeIn133_cxx
treeIn133::treeIn133(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("decay/decay_brips3107.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("decay/decay_brips3107.root");
      }
      f->GetObject("treeIn133",tree);
...
Int_t treeIn133::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t treeIn133::LoadTree(Long64_t entry)
{
...
void treeIn133::Init(TTree *tree)
{
...
void treeIn133::Show(Long64_t entry)
{
...
Int_t treeIn133::Cut(Long64_t entry)
{
