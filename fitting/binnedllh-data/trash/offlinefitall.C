...
#include "TLegend.h"
#include "TGraph.h"
void offlinefitall(Int_t rebin, Int_t niter, char* ri)
{
  gROOT->ProcessLine(".L fitDecayLL3hists_syserr.C");
  gROOT->ProcessLine(Form("fitDecayLL3hists_wrndcoin(\"%s\",\"../../../../../results/fithistograms/%slowin.root\",\"parms/parms%s.txt\",%d,\"fitresults/fitresult%slowin.root\",%d)",ri,ri,ri,niter,ri,rebin));
