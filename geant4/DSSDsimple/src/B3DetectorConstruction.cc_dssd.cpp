  ...
  fDetectorMessenger(0)
{
  ...
  fDetectorMessenger = new DetectorMessenger(this);
    detGroup=new G4int[10000];
    tubeGroup=new G4int[10000];
    x=new G4double[10000];
    y=new G4double[10000];
    z=new G4double[10000];
...
G4VPhysicalVolume* B3DetectorConstruction::Construct()
{
  ...
  G4Material* Vacuum = new G4Material("Galactic", 1., 1.01*g/mole,
    ...
    G4Material* STL= nist->FindOrBuildMaterial("G4_Si");
    ...
    STL -> AddElement(elSi,100.*perCent);
    */

    //-------------------Define Geometry---------------

    G4double moderator_sizeX=10*cm;
    G4double moderator_sizeY=10*cm;
    G4double moderator_sizeZ=10*cm;
  //
  // World
  //
  ...
  G4VisAttributes* Turquoise=
    ...
    //Turquoise->SetForceWireframe(true);
...
//--------------------------------------------------------------
    ...
    case 0: //UPC
    {
      G4LogicalVolume* He3Tube3=B3DetectorConstruction::DefineTubeGroup(i, 60.*cm,63.72*cm, 2.438/2*cm, 2.54/2*cm, 0.36*cm, He3matp8, STL);
        ...
        position=G4ThreeVector(x[i]*mm,y[i]*mm,z[i]*mm);
        break;
    }
    case 1: //GSI
    {
...
G4LogicalVolume* B3DetectorConstruction::DefineTubeGroup(G4int nTube, G4double He3Z,G4double Outer_Z, G4double He3R, G4double Outer_R,G4double He3OffsetPos, G4Material* He3mat, G4Material* Outermat)
{
  ...
  //
    G4Box* solidHe3Outer =
    new G4Box("He3out",  0.5*mm, 0.5*mm, 1.*mm);
