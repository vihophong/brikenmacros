...
void fitterBin(char* infilename,char* parmsfilename,char* outfilename,Double_t irejectrange=0.075, Double_t ineueff=0.62, Int_t ninterations=0,Int_t rebin=1)
{
    ...
    parms[knri*4+1]=hdecay->GetFunction("pol1")->GetParameter(0);
    parmserr[knri*4+1]=hdecay->GetFunction("pol1")->GetParError(0);
    parmsmin[knri*4+1]=parms[knri*4+1]-parms[knri*4+1]*bkgactmaxmin;
    parmsmax[knri*4+1]=parms[knri*4+1]+parms[knri*4+1]*bkgactmaxmin;

    parms[knri*4+7]=-hdecay->GetFunction("pol1")->GetParameter(1);
    parmserr[knri*4+7]=hdecay->GetFunction("pol1")->GetParError(1);
    ...
    hdecay1n->Fit("pol1","LQRE0","goff",lowerlimit,0.);

    parms[knri*4+2]=hdecay1n->GetFunction("pol1")->GetParameter(0);
    parmserr[knri*4+2]=hdecay1n->GetFunction("pol1")->GetParError(0);
    parmsmin[knri*4+2]=parms[knri*4+2]-parms[knri*4+2]*bkgactmaxmin;
    parmsmax[knri*4+2]=parms[knri*4+2]+parms[knri*4+2]*bkgactmaxmin;

    parms[knri*4+8]=-hdecay1n->GetFunction("pol1")->GetParameter(1);
    parmserr[knri*4+8]=-hdecay1n->GetFunction("pol1")->GetParError(1);

    //background of decay 2 neutrons gate

    hdecay2n->Fit("pol1","LQRE0","goff",lowerlimit,0.);

    parms[knri*4+3]=hdecay2n->GetFunction("pol1")->GetParameter(0);
    parmserr[knri*4+3]=hdecay2n->GetFunction("pol1")->GetParError(0);
    parmsmin[knri*4+3]=parms[knri*4+3]-parms[knri*4+3]*bkgactmaxmin;
    parmsmax[knri*4+3]=parms[knri*4+3]+parms[knri*4+3]*bkgactmaxmin;

    parms[knri*4+9]=-hdecay2n->GetFunction("pol1")->GetParameter(1);
    parmserr[knri*4+9]=-hdecay2n->GetFunction("pol1")->GetParError(1);
...
//        std::copy(mcparms.begin(),mcparms.end(),arr2);
    ...
    hdecay1n->GetFunction("pol1")->SetLineColor(kYellow);
    hdecay1n->GetFunction("pol1")->Draw("same");
    ...
    hdecay2n->GetFunction("pol1")->SetLineColor(kYellow);
    hdecay2n->GetFunction("pol1")->Draw("same");
