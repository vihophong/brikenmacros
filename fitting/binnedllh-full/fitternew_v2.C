 ...
 *A Constant background is adoptted
 *Nov 6, 2018
...
Bool_t reject=true;
Double_t rejectrange=0.075;//first 75 ms
...
struct GlobalChi2 {
   ...
   double operator() (const double *par) const {
      double p1[knri*3+2];
      for (int i = 0; i < knri*3+2; ++i) p1[i] = par[iparB[i] ];

      double p2[knri*3+4];
      for (int i = 0; i < knri*3+4; ++i) p2[i] = par[iparSB[i] ];

      double p3[knri*3+5];
      for (int i = 0; i < knri*3+5; ++i) p3[i] = par[iparSB2[i] ];
...
{    
    ...
    parms[knri*3+1]=bkgsum/((Double_t)bincnt);
    parmserr[knri*3+1]=parms[knri*3+1]*bkgactmaxmin;
    ...
    parms[knri*3+2]=bkgsum/((Double_t)bincnt);
    parmserr[knri*3+2]=parms[knri*3+2]*bkgactmaxmin;
    ...
    parms[knri*3+3]=bkgsum/((Double_t)bincnt);
    parmserr[knri*3+3]=parms[knri*3+3]*bkgactmaxmin;
    ...
    Double_t outparms[knri*3];
    Double_t outparmserr[knri*3];
    Int_t iparms[knri*3];
    Int_t isvary[knri*3];
    for (int i=0;i<knri*3;i++){
        outparms[i]=0;
    ...
    treeout->Branch("outparms",outparms,Form("outparms[%d]/D",knri*3));
    treeout->Branch("outparmserr",outparmserr,Form("outparmserr[%d]/D",knri*3));
    treeout->Branch("iparms",iparms,Form("iparms[%d]/I",knri*3));
    treeout->Branch("neueff",&neueff,"neueff/D");
    treeout->Branch("isvary",outparmserr,Form("isvary[%d]/I",knri*3));
    ...
    cout<<"\n\n*****SETTING PARAMETERS......\n"<<endl;
    fitter.Config().SetParamsSettings(knri*3+7,parms);
    for (int i=0;i<knri*3+7;i++){
        if (isparmsfix[i]){
    ...
    //!Perform the fit
    fitter.FitFCN(knri*3+7,globalChi2,0,dataB.Size()+dataSB.Size()+dataSB2.Size(),false);
    ...
    for (int i=0;i<knri*3+7;i++) mcparms[i]=parms[i];
    for(int i=0;i<ninterations;i++) {
        cout<<"mc "<<i+1<<endl;
        mc(rseed);
        fitter.Config().SetParamsSettings(knri*3+7,mcparms);
        for (unsigned int j=0;j<knri*3+7;j++){
        ...
        for (unsigned int i=0;i<knri*3+7;i++){
