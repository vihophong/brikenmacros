...
//Double_t neueff=0.702580;
Double_t neueff=0.628058639087;//average high in

//Double_t neueff=0.654478678054;//average low in

Bool_t reject=true;
Double_t rejectrangeall=0.1;//first bump
Double_t rejectrangeneugate=0.1;//first bump
...
Double_t fcn_gen(Double_t *x, Double_t *par) {
    ...
    //! reject 50 ms bump
    if (reject && x[0] > 0 && x[0] < rejectrangeall) {
       TF1::RejectPoint();
       return 0;
    }
    return returnval;
...
Double_t fcn_gen_wneutron(Double_t *x, Double_t *par) {
    ...
    //! reject 50 ms bump
    if (reject && x[0] > 0 && x[0] < rejectrangeneugate) {
       TF1::RejectPoint();
       return 0;
    }
...
void fitDecayLL3hists(char* fitname,char* infile,char* parmsfile, Int_t ninterations, char* outfile){
   ...
   //! write to text file
   std::ofstream ofs("fitresult.txt", std::ofstream::out | std::ofstream::app);
   ofs<<fitname<<",";
